...
use pokerust::{
    APIResourceList, Berry, BerryFirmness, BerryFlavor, BerryFlavorMap, FlavorBerryMap, Id,
    Language, Name, Named, NamedAPIResource, NamedAPIResourceList,
};
...
impl<T> From<APIResourceList<T>> for GraphedAPIResourceList {
    fn from(name: APIResourceList<T>) -> Self {
        GraphedAPIResourceList {
            count: name.count.to_string(),
            next: name.next,
            previous: name.previous,
            results: name
                .results
                .iter()
                .map(|r| GraphedAPIResource::from(r))
                .collect(),
        }
    }
}
